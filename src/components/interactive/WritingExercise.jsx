...
const normalizeCriteria = (criteria = []) => {
  ...
  return criteria
    .map((criterion) => criterion?.toString().trim())
    .filter(Boolean);
};
...
const getCriteria = (exercise = {}) =>
  normalizeCriteria(exercise.criteria || exercise.criterios);
...
const getGuide = (exercise = {}) =>
  exercise.guide || exercise.guia || "";

const buildBasicFeedback = ({ text, minWords, maxWords, criteria }) => {
  ...
  if (trimmedText.length > 0) {
    ...
    feedback.push({
      type: startsWithCapital ? "success" : "warning",
    ...
    feedback.push({
      type: endsWithPunctuation ? "success" : "warning",
  ...
  return {
    passed: !feedback.some((item) => item.type === "error"),
    feedback
  };
};
...
const WritingExercises = ({ ejercicios = [], onComplete }) => {
  ...
  useEffect(() => {
    if (!ejercicios.length || completed) return;

    const allReviewedAndPassed = ejercicios.every(
      (_, index) => reviews[index]?.passed === true
    );
    ...
    if (allReviewedAndPassed) {
      ...
      onComplete?.({
        ...
        completedExercises: ejercicios.length
      });
    }
  ...
  const handleReview = (index, exercise) => {
    const text = responses[index] || "";
    const minWords = getMinimumWords(exercise);
    const maxWords = getMaximumWords(exercise);
    const criteria = getCriteria(exercise);
  ...
  return (
    <>
      {ejercicios.map((exercise, index) => {
        const minWords = getMinimumWords(exercise);
        const maxWords = getMaximumWords(exercise);
        const suggestedTime = getSuggestedTime(exercise);
        const criteria = getCriteria(exercise);
          ...
          >
            ...
            <div className="flex flex-wrap gap-3 text-sm bg-blue-50 border border-blue-100 p-4 rounded-xl mb-4">
              <span className="font-semibold text-blue-800">
                Minimum: {minWords} słów
              </span>

              {maxWords && (
                <span className="text-blue-700">
                  Maksimum: {maxWords} słów
                </span>
              )}

              {suggestedTime && (
                <span className="text-blue-700">
                  Sugerowany czas: {suggestedTime} minut
                </span>
              )}
            </div>
              ...
              <div className="bg-gray-50 p-4 rounded-xl mb-4">
                ...
                <ul className="list-disc pl-5 space-y-1">
                  {criteria.map((criterion, criterionIndex) => (
                    <li key={criterionIndex} className="text-gray-700">
                      {criterion}
                    </li>
                  ))}
                </ul>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between gap-2 text-sm mt-3">
              ...
              <span className="text-gray-600">
                Wymagane minimum: {minWords} słów
              </span>
            </div>
            ...
            <div className="mt-4 flex flex-col sm:flex-row gap-3">
              <button
                type="button"
                onClick={() => handleReview(index, exercise)}
              ...
              {review && !review.passed && (
                <span className="text-sm text-gray-500 self-center">
                  Popraw odpowiedź i sprawdź ponownie.
                </span>
              )}
            </div>
            ...
            <div className="mt-4 p-3 rounded-xl bg-gray-50 text-sm text-gray-600">
              To jest podstawowa kontrola. Szczegółowa korekta gramatyki,
              słownictwa i stylu może zostać dodana później przez AI lub
              nauczyciela.
            </div>
          </div>
        );
