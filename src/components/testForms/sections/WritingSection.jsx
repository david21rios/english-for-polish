...
const getQuestionStatus = (question) => {
  ...
  const hasMinWords = Number(normalizedQuestion.minWords) > 0;
  const hasCriteria = normalizedQuestion.evaluationCriteria.some((criteria) =>
    criteria.trim()
  );
...
const WritingSection = ({
  ...
  const removeQuestion = (index) => {
    const confirmDelete = window.confirm(
      "¿Deseas eliminar esta pregunta de escritura?"
    );
  ...
  const updateEvaluationCriteria = (questionIndex, criteriaIndex, value) => {
    const updatedQuestions = safeQuestions.map((question, currentIndex) => {
      if (currentIndex !== questionIndex) return question;

      const updatedCriteria = question.evaluationCriteria.map(
        (criteria, currentCriteriaIndex) =>
          currentCriteriaIndex === criteriaIndex ? value : criteria
      );
  ...
  const removeEvaluationCriteria = (questionIndex, criteriaIndex) => {
    const updatedQuestions = safeQuestions.map((question, currentIndex) => {
      if (currentIndex !== questionIndex) return question;

      return {
        ...question,
        evaluationCriteria: question.evaluationCriteria.filter(
          (_, currentCriteriaIndex) => currentCriteriaIndex !== criteriaIndex
        )
      };
    });
  ...
  const updateKeywordCategory = (
    ...
    const updatedQuestions = safeQuestions.map((question, currentIndex) => {
      ...
      const updatedCategories = question.keywordCategories.map(
  ...
  const removeKeywordCategory = (questionIndex, categoryIndex) => {
    const updatedQuestions = safeQuestions.map((question, currentIndex) => {
      if (currentIndex !== questionIndex) return question;

      return {
        ...question,
        keywordCategories: question.keywordCategories.filter(
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm md:text-base text-gray-600 mt-2 leading-relaxed">
                Diseña tareas de producción escrita con criterios de evaluación,
                rango sugerido de palabras y categorías de vocabulario clave.
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-5">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-3 col-span-2 md:col-span-1">
            ...
            <p className="text-sm font-semibold text-gray-900 mt-1">
              Pregunta + palabras mínimas + al menos 1 criterio
            </p>
          </div>
        </div>
      </header>
      ...
      {safeQuestions.length === 0 ? (
        <div className="bg-white border border-dashed border-gray-300 rounded-3xl p-8 md:p-10 text-center">
          ...
          <h4 className="text-xl font-bold text-gray-900">
            No hay preguntas de escritura
          </h4>

          <p className="text-gray-600 mt-2 max-w-md mx-auto">
            Añade una tarea escrita para evaluar producción, vocabulario,
            coherencia y gramática.
          </p>
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <h4 className="font-bold text-gray-900 text-base md:text-lg">
                          Tarea escrita {questionIndex + 1}
                        </h4>

                        <p className="text-xs md:text-sm text-gray-500">
                          Prompt, ejemplo, palabras y criterios.
                        </p>
                      </div>
                    </div>
                ...
                <div className="p-4 md:p-5 space-y-5">
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Ejemplo: Write a short email introducing yourself to a new classmate."
                      rows="3"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Ejemplo orientativo para el evaluador o para pruebas internas."
                      rows="3"
                      className="w-full border border-gray-300 rounded-2xl px-4 py-3 text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-green-500 resize-none"
                      disabled={disabled}
                    />
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4">
                    <div className="flex items-start gap-2">
                      ...
                      <p className="text-sm text-blue-800 leading-relaxed">
                        En una versión posterior, estos criterios podrán ser
                        usados por IA para evaluar coherencia, gramática,
                        vocabulario, cumplimiento de tarea y nivel CEFR.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                      ...
                      <button
                        type="button"
                        onClick={() => addEvaluationCriteria(questionIndex)}
                        disabled={disabled}
                    ...
                    {question.evaluationCriteria.length === 0 ? (
                      <div className="bg-yellow-50 border border-yellow-100 text-yellow-800 rounded-2xl px-4 py-3 text-sm flex items-start gap-2">
                        ...
                        <p>
                          Añade al menos un criterio para que esta tarea quede
                          completa.
                        </p>
                      </div>
                    ) : (
                          ...
                          (criteria, criteriaIndex) => (
                            ...
                            >
                              <input
                                ...
                                placeholder="Ejemplo: Uses complete sentences."
                                disabled={disabled}
                              />
                  ...
                  <div className="space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Útiles para evaluar vocabulario esperado.
                        </p>
                      </div>

                      <button
                        type="button"
                        onClick={() => addKeywordCategory(questionIndex)}
                        disabled={disabled}
                        className="inline-flex items-center justify-center gap-2 text-sm font-semibold text-green-700 hover:text-green-800 disabled:opacity-50"
                      >
                        <FaPlus />
                    ...
                    {question.keywordCategories.length === 0 ? (
                      <div className="bg-gray-50 border border-gray-100 rounded-2xl px-4 py-3 text-sm text-gray-600 flex items-start gap-2">
                        ...
                        <p>
                          No hay categorías de palabras clave. Esto es opcional
                          por ahora.
                        </p>
                      </div>
                    ) : (
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <input
                                  ...
                                  className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-green-500 bg-white"
                                  placeholder="Ejemplo: Family vocabulary"
                                  disabled={disabled}
                                />
                              ...
                              <textarea
                                value={getKeywordsText(category)}
                                onChange={(event) =>
                                  updateKeywordCategory(
                                    questionIndex,
                                    categoryIndex,
