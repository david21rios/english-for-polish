...
const Temas = () => {
  ...
  const generateLines = () => {
    ...
    return circlesRef.current.slice(0, -1).map((circle, index) => {
      if (!circle || !circlesRef.current[index + 1]) {
        return null;
      }

      const rect1 = circle.getBoundingClientRect();
      const rect2 = circlesRef.current[index + 1].getBoundingClientRect();
          ...
          {loading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-gray-600 text-sm">
                An administrator can create topics from the admin panel.
              </p>
            </div>
          ) : (
