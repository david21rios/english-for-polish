...
| DOM-AUTH-003 | authorization / `CAPABILITY_IDS` / enum-like map | `authorization/capabilities.js` | 37 exact action IDs at R3-E gate (35 historical plus the previously reconciled cancel-self and restore contracts) | workflows, matrices, future backend; P,B | Domain docs; string mentions | ALREADY_EXTRACTED → `domain/capabilities.js` in R3-E | yes; same/order/freeze; restore; critical; scopes; E completed |
| DOM-AUTH-004 | authorization / `CAPABILITIES` / metadata map | same | 37 `{id,scope,resource,description}` records at R3-E gate | future authorization/backend; P,B | Domain docs | ALREADY_EXTRACTED → `domain/capabilities.js` in R3-E | yes; referential IDs/deep freeze; restore; critical; scopes+IDs; E completed |
| DOM-AUTH-005 | authorization / `IDENTITY_SELF_CAPABILITIES` / set | `authorization/identitySelfCapabilities.js` | six ordered IDs | future resolver; P,B | docs | ALREADY_EXTRACTED → `domain/identitySelfCapabilities.js` in R3-E | yes; same identity/order; restore; high; IDs; E completed |
| DOM-AUTH-006 | authorization / `ROLE_CAPABILITY_MATRIX` / matrix | `authorization/roleCapabilityMatrix.js` | 3 membership roles + platform_admin arrays | future resolver/backend; P,R semantics,B | docs, Rules independently enforce subset | ALREADY_EXTRACTED → `domain/roleCapabilityMatrix.js` in R3-E | yes; deep freeze/order; restore; critical; roles+IDs; E completed |
...
| DOM-ACAD-001 | academic / `CEFR_LEVELS` / enum | `academic/enums.js` | A1,A2,B1,B2,C1,C2 | Course validation and legacy learning UI/services; P,F,B,U | unit tests; legacy literal copies | ALREADY_EXTRACTED → `domain/course.js` in R3-F | yes; same/order; restore; high; legacy audit completed without cutover; F completed |
...
| DOM-WF-002 | workflow / `TENANT_WORKFLOW` / state machine | `workflow/tenantWorkflow.js` | active initial; 4 transitions; archived terminal | future commands; P,F,R,B | Domain docs | DOMAIN_MUST_REMAIN_TEMPORARY_AUTHORITY | no in R3-D; complete workflow mixes lifecycle, actors and capabilities; revisit in R3-H/separate resolution; critical; status+actors+caps; H |
| DOM-WF-003 | workflow / `REGISTRATION_REQUEST_WORKFLOW` / state machine | `workflow/registrationRequestWorkflow.js` | pending initial; 4 terminal transitions | future commands; P,F,R,B | Domain docs/runtime semantics | DOMAIN_MUST_REMAIN_TEMPORARY_AUTHORITY | no in R3-D; complete workflow mixes lifecycle, actors and capabilities; revisit in R3-H/separate resolution; critical; statuses+actors+caps; H |
| DOM-WF-004 | workflow / `MEMBERSHIP_WORKFLOW` / state machine | `workflow/membershipWorkflow.js` | approved initial; 4 transitions | future commands; P,F,R,B | Domain docs/runtime semantics | DOMAIN_MUST_REMAIN_TEMPORARY_AUTHORITY | no in R3-D; complete workflow mixes lifecycle, actors and capabilities; independent `MEMBERSHIP_STATUS_TRANSITIONS` remains R3-D; critical; roles/status/caps; H |
| DOM-WF-005 | workflow / `COURSE_WORKFLOW` / state machine | `workflow/courseWorkflow.js` | draft initial; 3 transitions | future commands; P,F,R,B | Domain docs/runtime semantics | DOMAIN_MUST_REMAIN_TEMPORARY_AUTHORITY | no in R3-D; complete workflow mixes lifecycle, actors and capabilities; revisit in R3-H/separate resolution; critical; statuses/actors/caps; H |
...
| R3-D | existing pure access-state and lifecycle maps only: ID-003/004/005, ORG-005, ACAD-004; WF-002..006 are evidence/consumers, not migration targets | package lifecycle/access-state; Domain reexports | deep freeze/order/reference tests, workflow parity as unchanged consumers, repository lifecycle tests, Rules/build | critical; revert; maps byte/value-equivalent; no complete workflow migration and no invented symbols |
