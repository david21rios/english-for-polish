...
export const createTenantBootstrapTransactionStore=(runner:TransactionRunnerPort):TenantBootstrapTransactionStore=>Object.freeze({execute:async(input:BootstrapTenantAggregate)=>runAuthoritativeTransaction(runner,async({transaction})=>{
  ...
  writeAuditEvent(transaction,{...common,auditId:`${input.commandId}-tenant-create`,authority:input.actor,destination:{kind:"tenant",tenantId:input.tenantId}});writeAuditEvent(transaction,{...common,auditId:`${input.commandId}-platform-create`,authority:input.actor,destination:{kind:"platform"}});
