...
import type {
  ...
  UpdateTenantSettingsTransactionStore,
...
const exactAuditMap = (
  ...
  if (
    ...
    return contract(
      `UpdateTenantSettings ${label} violates its shared allowlist.`,
    );
  }
...
const validateActor = (
  ...
  if (!validation.ok) {
    return contract(
      "UpdateTenantSettings actor authority is malformed.",
    );
  }
  ...
  if (
    ...
    return contract(
      "UpdateTenantSettings actor authority is not authorized.",
    );
  }
};

export const validateUpdateTenantSettingsPersistedResult = (
  value: unknown,
  expected: Readonly<{
    commandId: string;
  ...
  const validation =
    validateUpdateTenantSettingsResult(value);

  if (!validation.ok) {
    return contract(
      "UpdateTenantSettings persisted result is malformed.",
    );
  }
  ...
  if (
    ...
    return contract(
      "UpdateTenantSettings persisted result binding is incoherent.",
    );
  }
};

export const createUpdateTenantSettingsTransactionStore = (
        ...
        async ({ transaction }) => {
          ...
          if (commandSnapshot.exists) {
            ...
            if (
              ...
              return conflict(
                "UpdateTenantSettings command binding conflicts.",
              );
            }
            ...
            if (
              ...
              return contract(
                "UpdateTenantSettings replay command state is malformed.",
              );
            }
            ...
            if (
              ...
              return contract(
                "UpdateTenantSettings command authority binding is incoherent.",
              );
            }
          ...
          const currentSettingsValidation =
            validateTenantSettings(
              settingsSnapshot.data,
            );

          if (
            !currentSettingsValidation.ok
          ) {
            return contract(
              "Persisted Tenant Settings are malformed.",
            );
          }

          const currentSettings =
            currentSettingsValidation.value as Readonly<Record<string, unknown>>;

          if (
            currentSettings.tenantId
              !== input.tenantId
          ) {
            return contract(
              "Persisted Tenant Settings binding is incoherent.",
            );
          }

          if (
            typeof currentSettings.version
              !== "number"
            || !Number.isInteger(
            ...
            return contract(
              "Persisted Tenant Settings version is malformed.",
            );
          }
          ...
          if (
            ...
            return conflict(
              "UpdateTenantSettings expectedVersion conflicts with persisted version.",
            );
          }
          ...
          if (!sharedInputValidation.ok) {
            return contract(
              "UpdateTenantSettings replacement violates the shared input contract.",
            );
          }
          ...
          if (
            ...
            return contract(
              "UpdateTenantSettings next version is invalid.",
            );
          }
          ...
          for (
            ...
            if (
              ...
              return contract(
                "UpdateTenantSettings replacement contains a server timestamp token.",
              );
            }
          }

          if (
            typeof currentSettings.updatedAt
              !== "string"
          ) {
            return contract(
              "Persisted Tenant Settings updatedAt is malformed.",
            );
          }
          ...
          const logicalSettingsValidation =
            validateTenantSettings(
              logicalSettings,
            );

          if (
            !logicalSettingsValidation.ok
          ) {
            return contract(
              "UpdateTenantSettings write violates the persisted Settings contract.",
            );
          }
          ...
          writeAuditEvent(
            ...
            {
              auditId:
                `${input.commandId}-tenant-settings-update`,
