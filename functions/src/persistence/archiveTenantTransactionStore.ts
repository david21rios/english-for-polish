...
export interface ArchiveTenantTransactionInput {
...
export interface ArchiveTenantTransactionStore {
  execute(
    input: ArchiveTenantTransactionInput,
  ): Promise<Readonly<{
    replayed: boolean;
  }>>;
}
...
const exactAuditMap = (
  ...
  if (
    ...
    return contract(
      `ArchiveTenant ${label} violates its shared allowlist.`,
    );
  }
...
const validateActor = (
  ...
  if (!validation.ok) {
    return contract(
      "ArchiveTenant actor authority is malformed.",
    );
  }
  ...
  if (
    ...
    return contract(
      "ArchiveTenant actor authority is incoherent.",
    );
  }
};
...
const validatePersistedResult = (
  ...
  if (!validation.ok) {
    return contract(
      "ArchiveTenant persisted result is malformed.",
    );
  }
  ...
  if (
    ...
    return contract(
      "ArchiveTenant persisted result binding is incoherent.",
    );
  }
};

export const createArchiveTenantTransactionStore = (
        ...
        async ({
          ...
          if (
            ...
            if (
              ...
              return conflict(
                "ArchiveTenant command binding conflicts.",
              );
            }
            ...
            if (
              ...
              return contract(
                "ArchiveTenant replay command state is malformed.",
              );
            }
            ...
            if (
              ...
              return contract(
                "ArchiveTenant command authority binding is incoherent.",
              );
            }
          ...
          if (
            ...
            return failedPrecondition(
              "Tenant cannot transition to archived from its current status.",
            );
          }
          ...
          if (!logicalValidation.ok) {
            return contract(
              "ArchiveTenant transition violates the persisted Tenant contract.",
            );
          }
          ...
          writeAuditEvent(
            ...
            {
              auditId:
                `${input.commandId}-tenant-archive`,
