...
export const resolveUpdateTenantBrandingAuthority = async (
  ...
  if (membership.role !== MEMBERSHIP_ROLES.TENANT_ADMIN) {
    return forbidden(
      "UpdateTenantBranding requires tenant_admin authority.",
    );
  }
  ...
  if (
    ...
    return contract(
      "tenant_admin authority is missing tenant.manage_branding.",
    );
  }
  ...
  if (!resolutionValidation.ok) {
    return contract(
      "UpdateTenantBranding AuthorityResolution is malformed.",
    );
  }
