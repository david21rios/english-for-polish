...
export const RESTORE_TENANT_REQUIRED_AUTHORITY =
  PLATFORM_ROLES.PLATFORM_ADMIN;

export const RESTORE_TENANT_REQUIRED_CAPABILITY =
  CAPABILITY_IDS.PLATFORM_TENANT_RESTORE;

export const resolveRestoreTenantAuthority = async (
  ...
  if (
    ...
    return forbidden(
      "RestoreTenant requires Platform Admin actor type.",
    );
  }
  ...
  if (
    ...
    return forbidden(
      "RestoreTenant requires Platform Admin authority.",
    );
  }

  if (authority.tenantId !== null) {
    return forbidden(
      "RestoreTenant requires global Platform Admin scope.",
    );
  }
  ...
  if (
    ...
    return contract(
      "RestoreTenant Platform Admin roles are malformed.",
    );
  }

  if (authority.roles.length !== 1) {
    return contract(
      "RestoreTenant Platform Admin role binding is incoherent.",
    );
  }
  ...
  if (
    ...
    return forbidden(
      "RestoreTenant requires platform.tenant_restore capability.",
    );
  }
  ...
  if (!validation.ok) {
    return contract(
      "RestoreTenant AuthorityResolution is malformed.",
    );
  }
