...
export const resolveUpdateTenantProfileAuthority = async (
  ...
  if (identitySnapshot.data.uid !== actor.uid) {
    return forbidden("The actor Identity is not coherent with authentication.");
  }

  const tenantSnapshot = await reader.read(tenantDocumentPath(tenantId), "tenant");
  ...
  if (
    ...
    return forbidden("MembershipKey authority is not approved for the authenticated actor.");
  }
  ...
  if (membership.role !== MEMBERSHIP_ROLES.TENANT_ADMIN) {
    return forbidden("UpdateTenantProfile requires tenant_admin authority.");
  }

  const capabilities = capabilitiesForMembershipRole(MEMBERSHIP_ROLES.TENANT_ADMIN);

  if (!capabilities.includes(CAPABILITY_IDS.TENANT_UPDATE)) {
    return contract("tenant_admin authority is missing tenant.update.");
  }
  ...
  const resolution = Object.freeze({
    ...
    tenantId,
    roles: Object.freeze([MEMBERSHIP_ROLES.TENANT_ADMIN]),
    capabilities,
  });

  const resolutionValidation = validateAuthorityResolution(resolution);

  if (!resolutionValidation.ok) {
    return contract("UpdateTenantProfile AuthorityResolution is malformed.");
  }
