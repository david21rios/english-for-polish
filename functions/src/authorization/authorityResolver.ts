...
export const resolvePlatformAuthority = async (reader: AuthoritativeReaderPort, actor: AuthenticatedActor): Promise<AuthorityResolution> => {
  ...
  if (!authority.exists || authority.data?.status !== PLATFORM_AUTHORITY_STATUSES.ACTIVE || authority.data.authority !== PLATFORM_ROLES.PLATFORM_ADMIN) {
...
export const resolveTenantAuthority = async (reader: AuthoritativeReaderPort, actor: AuthenticatedActor, tenantId: string, membershipId: string): Promise<AuthorityResolution> => {
  ...
  if (!tenant.exists || tenant.data?.status !== TENANT_STATUSES.ACTIVE) throw new BackendError(BACKEND_ERROR_CODES.FAILED_PRECONDITION, "An active Tenant is required.");
  ...
  if (membership.data.uid !== actor.uid || membership.data.tenantId !== tenantId || membership.data.status !== MEMBERSHIP_STATUSES.APPROVED || typeof membership.data.role !== "string") {
