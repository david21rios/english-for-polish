...
export const executeBootstrapPlatformAdmins=async(value:unknown,deps:BootstrapDependencies):Promise<BootstrapResult>=>{
  const input=validateBootstrapPlatformAdminsInput(value),operator=requireValidAuthorityResolution(deps.operator);if(operator.actorType!=="system"||operator.authority!=="platform_system")throw new BackendError(BACKEND_ERROR_CODES.CONTRACT_VIOLATION,"Bootstrap operator authority is invalid.");const effectiveDeps:BootstrapDependencies={...deps,operator};const behavioral:Readonly<Record<string,JsonValue>>={environment:input.environment,confirmationId:input.confirmationId,targets:input.targets.map(target=>({uid:target.uid,expectedNormalizedEmail:target.expectedNormalizedEmail}))}, envelope:CommandEnvelope={commandId:input.commandId,commandType:COMMAND_TYPES.BOOTSTRAP_PLATFORM_ADMINS,correlationId:input.correlationId,tenantId:null,payload:behavioral};validateCommandEnvelope(envelope);
  ...
  deps=effectiveDeps;const payloadHash=canonicalPayloadHash(envelope.commandType,envelope.payload), claimed=await claimCommand(deps,input,envelope,payloadHash);if(claimed.kind==="replay")return result(input,true);
